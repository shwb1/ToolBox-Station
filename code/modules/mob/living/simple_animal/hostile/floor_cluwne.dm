...
/mob/living/simple_animal/hostile/floor_cluwne/Life()
	...
	pixel_y = 8
	var/area/src_location = get_area(src.loc)
	if(is_type_in_typecache(src_location, invalid_area_typecache) || !is_station_level(z))
	...
	var/area/T_location = get_area(T)
	if(prob(5))//checks roughly every 20 ticks
		if(current_victim.stat == DEAD || current_victim.dna.check_mutation(CLUWNEMUT) || is_type_in_typecache(T_location, invalid_area_typecache) || !is_station_level(current_victim.z))
			if(!Found_You())
				Acquire_Victim()

	if(get_dist(src, current_victim) > 9 && !manifested &&  !is_type_in_typecache(T_location, invalid_area_typecache))//if cluwne gets stuck he just teleports
		do_teleport(src, T)
...
/mob/living/simple_animal/hostile/floor_cluwne/Goto(target, delay, minimum_distance)
	var/area/location = get_area(current_victim.loc)
	if(!manifested && !is_type_in_typecache(location, invalid_area_typecache) && is_station_level(current_victim.z))
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Acquire_Victim(specific)
	for(var/I in GLOB.player_list)//better than a potential recursive loop
		...
		var/area/location = get_area(H.loc)

		if(specific)
			H = specific
			if(H.stat != DEAD && H.has_dna() && !H.dna.check_mutation(CLUWNEMUT) && !is_type_in_typecache(location, invalid_area_typecache) && is_station_level(H.z))
				return target = current_victim

		if(H && ishuman(H) && H.stat != DEAD && H != current_victim && H.has_dna() && !H.dna.check_mutation(CLUWNEMUT) && !is_type_in_typecache(location, invalid_area_typecache) && is_station_level(H.z))
			current_victim = H
			interest = 0
			stage = STAGE_HAUNT
			return target = current_victim
