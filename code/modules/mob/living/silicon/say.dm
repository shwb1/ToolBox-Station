/mob/living/proc/robot_talk(message)
	...
	for(var/mob/M in GLOB.player_list)
		if(M.binarycheck())
			if(isAI(M))
				var/renderedAI = "<span class='binarysay'>Robotic Talk, <a href='?src=[REF(M)];speakerreference=[REF(src)];track=[Clean_up_hashtags(html_encode(name))]'><span class='name'>[name] ([desig])</span></a> <span class='message'>[message_a]</span></span>"
