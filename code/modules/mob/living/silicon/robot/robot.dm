...
/mob/living/silicon/robot/proc/notify_ai(notifytype, oldname, newname)
	...
	switch(notifytype)
		if(NEW_BORG) //New Cyborg
			to_chat(connected_ai, "<br><br><span class='notice'>NOTICE - New cyborg connection detected: <a href='?src=[REF(connected_ai)];speakerreference=[REF(src)];track=[Clean_up_hashtags(html_encode(name))]'>[name]</a></span><br>")
		...
		if(AI_SHELL) //New Shell
			to_chat(connected_ai, "<br><br><span class='notice'>NOTICE - New cyborg shell detected: <a href='?src=[REF(connected_ai)];speakerreference=[REF(src)];track=[Clean_up_hashtags(html_encode(name))]'>[name]</a></span><br>")
