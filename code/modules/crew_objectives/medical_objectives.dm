...
/datum/objective/crew/morgue/check_completion()
	...
	for(var/mob/living/carbon/human/H in GLOB.mob_living_list)
		var/area/location = get_area(H)
		if(H.stat == DEAD && is_station_level(H.z) && is_type_in_typecache(location, medical_areas)) // If person is dead and corpse is in one of these areas
			return FALSE
	return TRUE
...
/datum/objective/crew/emtmorgue/check_completion()
	...
	for(var/mob/living/carbon/human/H in GLOB.mob_living_list)
		var/area/location = get_area(H)
		if(H.stat == DEAD && is_station_level(H.z) && !is_type_in_typecache(location, medical_areas_morgue)) // If person is dead and corpse is NOT in one of these areas
			return FALSE
	return TRUE
