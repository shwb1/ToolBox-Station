...
var/const/EP_MAX_SINGLE_STACK = 7 * 12
var/const/EP_MAX_AOE_STACK = 7 * 12
	...
	*/
	...
	attack_hand(var/mob/living/M)
		if(cell && (M.held_items[1] == src || M.held_items[2] == src))
			...
			cell = 0
			update_icon()
			M.equip_to_slot_if_possible(I, SLOT_HANDS)
			return
		. = ..()
	...
	screwdriver_act(mob/living/user, obj/item/I)
		panel = !panel
		to_chat(user, "<div class='danger'>You [panel ? "open" : "close"] the maintenance panel.</div>")
		playsound(loc, 'sound/items/Screwdriver.ogg', 50, 1)
		return ..()

	welder_act(mob/living/user, obj/item/I)
		if(emagged && panel && check_perseus(user))
			var/obj/item/weldingtool/WT = I
			if(!WT.use(0))
				return
			playsound(loc, 'sound/items/Welder2.ogg', 40, 1)
			if(do_after(user, 20, target = user))
				to_chat(user, "<div class='danger'>You repair the [src].</div>")
				emagged = 0
...
/obj/item/projectile/energy/ep90_single
	...
	on_hit(var/atom/target, var/blocked = 0)
		...
		if(IS_EP_SINGLE_STACKING && !emagged && !check_perseus(L))
			...
			if((L.AmountParalyzed() + knockdown) > max)	L.SetParalyzed(max)
			else							L.AdjustParalyzed(knockdown)
...
/obj/item/projectile/energy/ep90_aoe
	...
	can_hit_target(atom/target, list/passthrough, direct_target = FALSE, ignore_loc = FALSE)
		if(loc == original)
			return TRUE
		return ..()

	fire(angle, atom/direct_target)
		original = get_turf(original)
		return ..()
