...
/obj/machinery/computer/percsecuritysystem/process()
	...
	for(var/mob/living/carbon/human/perchuman in GLOB.mob_list)
...
/obj/machinery/computer/percsecuritysystem/attack_hand(mob/living/user as mob)
	...
	if(shiparea)
		for(var/turf/open/floor/percfloor in shiparea)
			var/hasenginepart = 0
	...
	var/shuttlestatus = "Shuttle Destroyed"
	if(perseusshuttle)
		shuttlestatus = perseusshuttle.getStatusText()
...
/obj/machinery/computer/percsecuritysystem/examine()
	...
	if(!istype(usr,/mob/living))
		. += "All you see on the screen are strange green numbers falling down from top to bottom like rain."
	else
...
/obj/machinery/computer/percsecuritysystem/proc/intruderalert(intruders = 0)
	...
	if (!A)
		return
	if(!intruder_overlay)
		intruder_overlay = iconstate2appearance('icons/turf/areas.dmi', "bluenew")
	var/perseus_entry = "PERSEUSINTRUDERS"
	if(intruders)
		if(!(perseus_entry in A.customalarms))
			A.customalarms += perseus_entry
			A.add_overlay(intruder_overlay)
	else
		if(perseus_entry in A.customalarms)
			A.customalarms -= perseus_entry
			A.cut_overlay(intruder_overlay)