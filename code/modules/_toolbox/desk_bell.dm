...
/obj/structure/desk_bell/screwdriver_act(mob/living/user, obj/item/I)
	if(broken_ringer)
		to_chat(user, "<span class='notice'>You beging to repair the [src]...</span>")
		I.play_tool_sound(src)
		if(I.use_tool(src, user, 5 SECONDS))
			user.visible_message("[user] repaired the [src] with the [I].", \
				"<span class='notice'>You repair the [src].</span>", \
				"<span class='italics'>You hear a screwdriver sound.</span>")
...
/obj/structure/desk_bell/wrench_act(mob/living/user, obj/item/I)
	to_chat(user, "<span class='notice'>You beging to disesemble the [src]...</span>")
	I.play_tool_sound(src)
	if(I.use_tool(src, user, 5 SECONDS))
