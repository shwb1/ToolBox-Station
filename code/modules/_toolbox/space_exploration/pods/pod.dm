...
/obj/pod
	...
	var/datum/action/innate/pod/openhud/OpenHUD = new
	var/datum/action/innate/pod/leavepod/LeavePod = new

	var/static/radial_hud = image(icon = 'icons/mob/actions/actions_mecha.dmi', icon_state = "mech_view_stats")
	var/static/radial_enter = image(icon = 'icons/mob/actions/actions_mecha.dmi', icon_state = "mech_overload_off")
	var/static/radial_eject = image(icon = 'icons/mob/actions/actions_mecha.dmi', icon_state = "mech_eject")
	...
	Initialize()
		...
		if(!GLOB.pod_config)
			GLOB.pod_config = new /datum/space_exploration_config/pod()
	...
	examine()
		...
		switch(hp)
			...
			if(26 to 50)
				. += "<span class='warning'>It looks significantly damaged.</span>"
			if(51 to 75)
				. += "<span class='warning'>It looks moderately damaged.</span>"
			if(76 to 99)
				. += "<span class='warning'>It looks slightly damaged.</span>"
			if(100 to INFINITY)
				. += "<span class='info'>It looks undamaged.</span>"

		. += "<span class='info'>Attached are:</span>"
		for(var/obj/item/pod_attachment/attachment in GetAttachments())
			if(attachment.hardpoint_slot in list(P_HARDPOINT_PRIMARY_ATTACHMENT, P_HARDPOINT_ARMOR, P_HARDPOINT_SHIELD, P_HARDPOINT_SECONDARY_ATTACHMENT))
				. += "<span class='info'>- \The [attachment.name]"
	...
	proc/HandleExit(var/mob/living/carbon/human/H)
		...
		if(do_after(H, exit_delay, target = src))
	...
	proc/DoMove(var/mob/user, var/_dir)
		...
		var/drifting = 0
		var/propulsion = 0
		var/the_move_cooldown_modifier = 0
		var/obj/item/pod_attachment/engine/E = GetAttachmentOnHardpoint(P_HARDPOINT_ENGINE)
		if(E)
			propulsion = E.propulsion
			the_move_cooldown_modifier = E.pod_move_reduction
		if(!propulsion)
			...
			inertial_direction = 0
		if(!drifting || propulsion)
			var/the_move_cooldown = move_cooldown
			if(drifting) //propulsion only works in space
				the_move_cooldown = max(the_move_cooldown+the_move_cooldown_modifier,0.1)
			if((pod_last_move_time + the_move_cooldown) > world.time)
				return 0
			var/thestep = get_step(loc, _dir)
			if(thestep && !(_dir & (_dir - 1)))
				Move(thestep, _dir)
	...
	attack_hand(var/mob/living/user)
		if(user.a_intent == "grab")
			...
			if(!length(possible_targets))
				to_chat(user,"<span class='warning'>There is noone inside. (Cancel Grab intent to access hud)</span>")
				return 0
			...
			if(do_after(user, GLOB.pod_config.pod_pullout_delay,target = src))
				if(chosen_mob && (chosen_mob in GetOccupants()))
					...
					chosen_mob.forceMove(get_turf(src))
					if(is_pilot)
						RemoveActions(pilot)
						pilot = 0
	...
	proc/GrantActions(mob/living/user)
		if(!pilot)
			return
		OpenHUD.Grant(user, src)
		LeavePod.Grant(user, src)

	proc/RemoveActions(mob/living/user)
		if(!pilot)
			return
		OpenHUD.Remove(user)
