		...
		if(istype(I, /obj/item/weldingtool))
			...
			if(W.isOn())
				if(prob(75))
					...
					S = new /obj/item/stack/sheet/iron(get_turf(src))
...
/obj/pod
	...
	attack_animal(var/mob/living/simple_animal/animal)
		TakeDamage(animal.melee_damage, 0, 0, animal)
	...
	process(var/obj/pod/pod)
		...
		if(pod.HasDamageFlag(P_DAMAGE_FIRE) && ((last_fire_tick + GLOB.pod_config.fire_damage_cooldown) <= world.time))
			...
			if(istype(get_turf(pod), /turf/open/space))
				if(!pod.internal_air || pod.internal_air.get_moles(/datum/gas/oxygen) < GLOB.pod_config.fire_damage_oygen_cutoff)
				...
				pod.internal_air.set_moles(/datum/gas/oxygen, pod.internal_air.get_moles(/datum/gas/oxygen) - (pod.internal_air.get_moles(/datum/gas/oxygen) * GLOB.pod_config.fire_oxygen_consumption_percent))
