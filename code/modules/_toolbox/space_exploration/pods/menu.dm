/obj/pod/
	...
	Topic(href, href_list)
		...
		if((toggles & P_TOGGLE_HUDLOCK) && usr != pilot)
			to_chat(usr,"<span class='warning'>The operator has forbidden outside menu access.</span>")
			return 0
		...
		switch(href_list["action"])
			if("remove_canister")
				...
				if(do_after(usr, 20,target = src))
					...
					internal_canister.forceMove(get_turf(usr))
					internal_canister = 0
			...
			if("remove_battery")
				...
				if(do_after(usr, 10,target = src))
					...
					power_source.forceMove(get_turf(usr))
					power_source = 0
