/obj/item/melee/baton
	...
	force = 10 //MERGE Wants this to be 8, keeping it at 10.
	throwforce = 7
...
/obj/item/melee/baton/attack(mob/M, mob/living/carbon/human/user)
	if(turned_on && HAS_TRAIT(user, TRAIT_CLUMSY) && prob(50) && !(obj_flags & OBJ_EMPED))
		...
		user.Paralyze(stunforce*2)
		//user.adjustStaminaLoss(stunforce*3)
		/* MERGE wants this to be stam loss, comment up above says it shouldnt be, keeping old code.
		user.adjustStaminaLoss(stunforce*3)
		user.stuttering = 20
...
/obj/item/melee/baton/proc/baton_stun(mob/living/target, mob/living/user)
	...
	target.Paralyze(stunforce) //we dont gay up our combat.
